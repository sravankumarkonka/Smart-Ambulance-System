...
class LoadExcelReporter:
    def __init__(self, filename="Load_Performance_300_Test_Report.xlsx"):
    ...
    def generate_report(self, test_results, start_time, end_time):
        ...
        ws_dash.title = "Load Performance Summary"
        ws_dash.views.sheetView[0].showGridLines = True

        # Header Title Banner
        ws_dash.merge_cells("A1:G2")
        title_cell = ws_dash["A1"]
        title_cell.value = "Smart Ambulance System - Load & Performance Benchmark Report (300 Test Cases)"
        ...
        headers_metrics = [
            ("Total Load Tests", total_tests, "1E293B"),
            ("SLA Met / Passed", passed, "15803D"),
            ("SLA Breached / Failed", failed, "B91C1C" if failed > 0 else "475569"),
            ("Skipped Tests", skipped, "D97706" if skipped > 0 else "475569"),
            ("SLA Compliance Rate", f"{pass_rate:.2f}%", "166534"),
            ("Avg Benchmark Latency", f"{avg_latency*1000:.2f} ms", "0284C7")
        ]
        ...
        ws_dash["A7"] = "Load & Performance Category Breakdown"
        ws_dash["A7"].font = Font(name="Calibri", size=12, bold=True, color="0F172A")

        table_headers = ["Performance Category", "Tests Run", "SLA Passed", "Failed", "Skipped", "SLA Pass Rate"]
        ...
        # --- Sheet 2: Load Test Detailed Results ---
        ws_details = self.wb.create_sheet(title="Load Test Detailed Results")
        ws_details.views.sheetView[0].showGridLines = True

        detail_headers = [
            "Test ID", "Performance Category", "Test Name", "Load Metric / Target Description", 
            "Target SLA Threshold", "Actual Latency / Throughput", "Status", "Duration (s)", "Timestamp", "Performance Log Detail"
        ]
        ...
        print(f"[LoadExcelReporter] Load Test Report saved to '{self.filename}' ({total_tests} load tests).")
