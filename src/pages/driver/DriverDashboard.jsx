    ...
    const initDriver = async () => {
      try {
        ...
        unsubscribeDriver = subscribeToDriverEmergencies(currentUser.uid, (list) => {
          const ACTIVE_STATUSES = ['assigned', 'accepted', 'on_the_way', 'reached', 'arrived', 'patient_picked', 'hospital_reached'];
          const active = (list || []).filter(e => e?.status && ACTIVE_STATUSES.includes(e.status));
    ...
    <div className="container mt-4" data-testid="driver-dashboard">
      <div className="mb-4" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <Link to="/driver/history" className="btn btn-outline" style={{ display: 'inline-flex', padding: '10px 20px' }}>
            📜 Response History
          </Link>
