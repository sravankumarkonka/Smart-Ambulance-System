...
const EmergencyHistory = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        const data = await getEmergencyHistory(uid);
        setHistory(data || []);
      } catch (err) {
        console.warn('[EmergencyHistory] History fetch warning:', err.message);
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'Waiting':
      case 'waiting':
        return <span className="badge badge-warning">Waiting for Driver</span>;
      case 'assigned':
