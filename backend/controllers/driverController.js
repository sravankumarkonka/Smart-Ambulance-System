...
export const assignDriver = async (req, res) => {
  try {
    ...
    // Update emergency request with merge: true to avoid NOT_FOUND errors
    ...
    batch.set(notifRef, {
      ...
      body: `Ambulance driver ${driverName} (${driverPhone}) has accepted your emergency request.`,
...
export const updateStatus = async (req, res) => {
  try {
    ...
    const allowedStatuses = ['assigned', 'accepted', 'on_the_way', 'reached', 'patient_picked', 'hospital_reached', 'arrived', 'completed', 'cancelled'];
    ...
    const notifTitles = {
      ...
      'reached': 'Driver Arrived at Incident Location',
      'arrived': 'Driver Arrived at Incident Location',
      'patient_picked': 'Patient Picked Up',
      'hospital_reached': 'Hospital Facility Reached',
      'completed': 'Emergency Completed',
      'cancelled': 'Emergency Dispatch Cancelled'
    };

    const title = notifTitles[status] || `Emergency Status: ${status}`;
    const body = `Your emergency request status was updated to ${status.replace('_', ' ').toUpperCase()}`;
    ...
    await notifRef.set({
      ...
      title,
      body,
      recipientUid: emergencyData.userId || emergencyData.patientUid || '',
      status,
      createdAt: now,
      read: false
    });
...
export const updateLocation = async (req, res) => {
  try {
    ...
    const { latitude, longitude, heading, speed, status, emergencyId } = req.body;
    ...
    // 1. Update location & telemetry in ambulances fleet doc
    const ambulanceRef = db.collection('ambulances').doc(driverId);
    batch.set(ambulanceRef, {
      driverUid: driverId,
      driverId,
      latitude: lat,
      longitude: lng,
      heading: heading !== undefined ? parseFloat(heading) : 0.0,
      speed: speed !== undefined ? parseFloat(speed) : 42.5,
      status: status || 'busy',
      updatedAt: now,
      lastUpdated: now
    }, { merge: true });

    // 2. Link coordinates & telemetry directly inside current emergency doc if active
    if (emergencyId) {
      const emergencyRef = db.collection('emergencies').doc(emergencyId);
      batch.update(emergencyRef, {
        driverLatitude: lat,
        driverLongitude: lng,
        driverHeading: heading !== undefined ? parseFloat(heading) : 0.0,
        driverSpeed: speed !== undefined ? parseFloat(speed) : 42.5,
        updatedAt: now
      });
    }
