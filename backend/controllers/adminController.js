...
export const getStats = async (req, res) => {
  try {
    ...
    const ACTIVE_STATUSES = ['pending', 'Waiting', 'waiting', 'assigned', 'accepted', 'on_the_way', 'reached', 'arrived', 'patient_picked', 'hospital_reached'];
    const activeCount = emergencies.filter(e => ACTIVE_STATUSES.includes(e.status)).length;
    const criticalCount = emergencies.filter(e => ACTIVE_STATUSES.includes(e.status) && (e.severityLevel === 'critical' || e.severity === 'critical')).length;
