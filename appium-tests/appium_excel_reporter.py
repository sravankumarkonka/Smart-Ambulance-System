...
class AppiumExcelReporter:
    def __init__(self, filename="Appium_Mobile_300_Test_Report.xlsx"):
    ...
    def generate_report(self, test_results, start_time, end_time):
        ...
        # --- Sheet 1: Appium Executive Summary Dashboard ---
        ws_dash = self.wb.active
        ws_dash.title = "Appium Mobile Summary"
        ws_dash.views.sheetView[0].showGridLines = True

        # Header Title Banner
        ws_dash.merge_cells("A1:G2")
        title_cell = ws_dash["A1"]
        title_cell.value = "Smart Ambulance System - Appium Mobile Automation Test Report (300 Test Cases)"
        title_cell.font = Font(name="Calibri", size=16, bold=True, color="FFFFFF")
        title_cell.fill = PatternFill(start_color="3B82F6", end_color="1D4ED8", fill_type="solid")
        title_cell.alignment = Alignment(horizontal="center", vertical="center")

        # Summary Metrics Cards
        headers_metrics = [
            ("Total Mobile Tests", total_tests, "1E293B"),
            ("Passed Actions", passed, "15803D"),
            ("Failed Actions", failed, "B91C1C" if failed > 0 else "475569"),
            ("Skipped Tests", skipped, "D97706" if skipped > 0 else "475569"),
            ("Mobile Pass Rate", f"{pass_rate:.2f}%", "166534"),
            ("Avg Action Speed", f"{avg_latency*1000:.2f} ms", "0284C7")
        ]
        ...
        ws_dash["A7"] = "Mobile Module Automation Summary"
        ws_dash["A7"].font = Font(name="Calibri", size=12, bold=True, color="0F172A")

        table_headers = ["Mobile Module", "Tests Run", "Passed", "Failed", "Skipped", "Module Pass Rate"]
        ...
        # --- Sheet 2: Appium Detailed Test Results ---
        ws_details = self.wb.create_sheet(title="Appium Detailed Results")
        ws_details.views.sheetView[0].showGridLines = True

        detail_headers = [
            "Test ID", "Mobile Module", "Test Name", "Target Element / Gesture", 
            "Action Type", "Status", "Duration (s)", "Timestamp", "Appium Assertion / Driver Log"
        ]
        ...
        for row_idx, res in enumerate(test_results, start=2):
            ws_details.append([
                ...
                res['module'],
                res['name'],
                res.get('element', 'Mobile UI Element'),
                res.get('action', 'Tap / Verify'),
                res['status'],
        ...
        print(f"[AppiumExcelReporter] Appium Report saved to '{self.filename}' ({total_tests} mobile tests).")
